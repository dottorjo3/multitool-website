// 🔧 File: backend/tools/text-remove-emoji.js
// 🔗 Rimuove emoji da un testo

const EMOJI_REGEX = /[\u{1F300}-\u{1F9FF}]|[\u{2600}-\u{26FF}]|[\u{2700}-\u{27BF}]|[\u{1F1E0}-\u{1F1FF}]|[\u{1F600}-\u{1F64F}]|[\u{1F680}-\u{1F6FF}]|[\u{1F700}-\u{1F77F}]|[\u{1F780}-\u{1F7FF}]|[\u{1F800}-\u{1F8FF}]|[\u{1F900}-\u{1F9FF}]|[\u{1FA00}-\u{1FA6F}]|[\u{1FA70}-\u{1FAFF}]/gu;

module.exports = {
  async run({ params }) {
    const input = params.input?.trim() || '';
    
    if (!input) {
      throw new Error('Inserisci il testo da cui rimuovere gli emoji');
    }

    const cleaned = input.replace(EMOJI_REGEX, '');
    const removed = input.length - cleaned.length;
    
    return {
      original: input,
      cleaned,
      emojiRemoved: removed,
      originalLength: input.length,
      cleanedLength: cleaned.length,
    };
  },
};

