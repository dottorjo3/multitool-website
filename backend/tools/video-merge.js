// 🔧 File: backend/tools/video-merge.js
// 🔗 Unisce più video
...
module.exports = {
  async run({ params, filesMeta, requestId }) {
    if (!filesMeta || filesMeta.length < 2) {
      throw new Error('Carica almeno 2 video da unire');
    }
    ...
    try {
      ...
      const outputName = `${requestId}-merged.${path.extname(filesMeta[0].originalName).replace('.', '') || 'mp4'}`;
