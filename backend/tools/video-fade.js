// 🔧 File: backend/tools/video-fade.js
// 🔗 Applica fade in/out al video
...
module.exports = {
  async run({ params, filesMeta, requestId }) {
    ...
    try {
      ...
      const outputName = `${requestId}-fade.${path.extname(file.originalName).replace('.', '') || 'mp4'}`;
      ...
      if (fadeIn > 0 && fadeOut > 0) {
        ...
        filter = `fade=t=out:st=${duration - fadeOut}:d=${fadeOut}`;
      }
