// 🔧 File: backend/tools/image-invert.js
...
module.exports = {
  async run({ filesMeta, requestId }) {
    if (!filesMeta || filesMeta.length !== 1) {
      throw new Error('Carica una singola immagine da invertire');
    }
    ...
    const outputName = `${requestId}-invert.${safeExtension === 'jpg' ? 'jpg' : safeExtension}`;
    const outputPath = path.resolve(TMP_DIR, outputName);

    const pipeline = sharp(file.filePath).negate();
