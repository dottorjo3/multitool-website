...
Mappen RPMS (eller eventuelt DEBS) inneholder også en pakke kalt libreoffice7.6-freedesktop-menus-7.6.noarch.rpm(or libreoffice7.6-debian-menus_7.6.0.1-1_all.deb, respectively eller lignende). Dette er en pakke for alle Linux-distribusjoner som støtter spesifikasjonene/anbefalingene til Freedesktop.org (http://en.wikipedia.org/wiki/Freedesktop.org) og er gjort tilgjengelig for installasjon på Linux-distribusjoner som ikke er dekket av de foregående instruksjonene.
...
Når du sender et dokument via 'Fil - Send - E-postdokument' eller 'Fil - Send - E-post som PDF' kan det oppstå problemer (programmet krasjer eller henger). Dette er på grunn av Windows-systemfilen "Mapi" (Messaging Application Programming Interface) som forårsaker problemer i enkelte filversjoner. Problemet kan dessverre ikke begrenses til et bestemt versjonsnummer. For mer informasjon besøk https://www.microsoft.com for å søke etter "mapi dll" i Microsoft Kunnskaps Base.
...
Vi håper du har glede av å arbeide med LibreOffice 7.6 og at vi snart møter deg på nettet.
